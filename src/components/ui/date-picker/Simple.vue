<template>
    <div class="datePicker">
        <input type="text" :name="name" :placeholder="placeholder" v-model="state.date" ref="inputRef"
            @input="datePickerHandleChange" @focus="datePickerHandleShow(true, 'opened')"
        ...
        <section v-if="inline || state.showDatePicker" class="datePicker__section" @mousedown.prevent>
            <table>
                <div v-if="state.showMonthList" class="datePicker__div">
                    <button type="button" v-for="(month, index) in state.rangeNameMonths" :key="index"
                        :class="{ 'datePicker--active': state.month === ++index }" @click="chooseMonth(index)">
                        {{ month }}
                    </button>
                </div>
                <div :class="[
                    ...
                    <button type="button" v-for="(year, index) in state.years" :key="index"
                        :class="{ 'datePicker--active': state.year === year }"
                        :data-scroll="state.year === year ? 1 : ''" @click="chooseYear(year)">
                        {{ year }}
                    </button>
                </div>
                <thead>
                    <tr>
                        ...
                        <th class="datePicker__th__button">
                            <button type="button" @click="nextMonthHandle">
                ...
                <tbody>
                    <tr v-for="(item, index) in state.calcNumberWeeks(
                        ...
                        <td v-for="(day, idx) in state.tableDays(
                            state.daysInMonth(state.year, state.month),
                            state.prevMonthDays(
...
export default {
    ...
    setup(props, context) {
        ...
        const state = reactive({
            ...
            years: computed(() => {
                let years = [];
                let start = props.yearRange[0];
                let end = props.yearRange[1];
                while (start <= end) {
                    years.push(start++);
                }
                return years;
        ...
        onMounted(() => {
            if (context.attrs.modelValue) {
                ...
                state.year = Math.min(Math.max(Number(dateArr[0]), props.yearRange[0]), props.yearRange[1]);
                state.month = Number(dateArr[1]);
                state.selected = Number(dateArr[2]);
                state.date = context.attrs.modelValue;
            } else {
                state.year = Math.min(Math.max(jalal.year(), props.yearRange[0]), props.yearRange[1]);
            }
        });
...
.datePicker .datePicker__section {
    ...
    background: linear-gradient(321deg, rgba(255, 112, 40, 0.32) 0%, rgba(255, 112, 40, 0.68) 100%);
}
...
.datePicker table tr th:not(.datePicker__th__button):not(.datePicker__th):hover:not(th):not(.datePicker__td--disable) {
    background: #ffa64d;
    color: #fff;
    cursor: pointer;
}
